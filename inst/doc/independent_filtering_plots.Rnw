...
Here, we use for illustration the \Robject{pasillaGenes} dataset from the
...
the were measured for each of two biological conditions (normally growing cells and cells
treated with \fixme{RNAi} against the \emph{pasilla} gene).
...
nArrays = 3
set.seed(1969)
use = unlist(tapply(seq(along=ALL_bcrneg$mol.biol), ALL_bcrneg$mol.biol, sample, nArrays))
sampleDataset = ALL_bcrneg[, use]
@

...
Here, `ttest` is a function from the `r Biocpkg("genefilter")` package which provides a suitable wrapper around `t.test` from package `r Biocpkg("stats")`. Now we see that there are `r {sum(wh2)}` genes that satisfy the selection criterion.

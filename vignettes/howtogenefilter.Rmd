...
Here `f1` is a function that implies our "expression measure above 200 in at least 5 samples" criterion, the function `ffun` is the filtering function (which in this case consists of only one criterion), and we apply it using `genefilter`. There were genes that satisfied the criterion and passed the filter.
...
Here, `ttest` is a function from the `r Biocpkg("genefilter ")` package which provides a suitable wrapper around \texttt{t.test} from package \Rpackage{stats}. Now we see that there are `r (wh2)` genes that satisfy the selection criterion.
